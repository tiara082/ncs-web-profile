...
<div class="page-header">
    ...
    <p class="page-subtitle">Update konten artikel atau publikasi</p>
</div>
    ...
    <div class="col-md-10">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">Form Edit Content</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('contents.update', $content) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" value="{{ $content->creator->name ?? $content->creator->username ?? 'Unknown' }}" disabled>
                        <small class="text-muted">Dibuat oleh: {{ $content->creator->name ?? $content->creator->username ?? 'Unknown' }} pada {{ $content->created_at->format('d M Y H:i') }}</small>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('content_type') is-invalid @enderror" 
                            ...
                            <option value="research" {{ old('content_type', $content->content_type) == 'research' ? 'selected' : '' }}>Research</option>
                            <option value="publication" {{ old('content_type', $content->content_type) == 'publication' ? 'selected' : '' }}>Publication</option>
                        </select>
                        @error('content_type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        ...
                        <select class="form-select @error('categories') is-invalid @enderror" 
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" 
                                    {{ in_array($category->id, old('categories', $content->categories->pluck('id')->toArray())) ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('body') is-invalid @enderror" 
                                  id="body" name="body" rows="15" required>{{ old('body', $content->body) }}</textarea>
                        @error('body')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
